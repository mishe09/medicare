...
export default function AppointmentSchedule({ darkMode }) {
  const days = ["Mon", "Tue", "Wed", "Thu", "Fri"];
  const times = [
    "8:00 AM", "9:00 AM", "10:00 AM", "11:00 AM",
    "12:00 PM", "1:00 PM", "2:00 PM", "3:00 PM", "4:00 PM", "5:00 PM"
  ...
  return (
    <div
      className={`shadow-lg rounded-lg p-4 w-full h-[80vh] overflow-hidden transition-colors duration-500 ${
    ...
    >
      ...
      {/* Calendar Grid */}
      <div
        className={`grid grid-cols-[100px_repeat(5,1fr)] h-full border-t border-l ${
          darkMode ? "border-gray-700" : "border-gray-200"
        }`}
      >
        {/* Time Column */}
        <div className="flex flex-col">
          {times.map((time) => (
            <div
              key={time}
              className={`h-[60px] flex items-start justify-center text-xs p-1 ${
        ...
        {days.map((day) => (
          <div
            key={day}
            className={`border-r ${darkMode ? "border-gray-700" : "border-gray-200"}`}
          >
            <div
            ...
            {times.map((_, index) => (
              <div
                key={index}
                className={`border-b h-[60px] transition cursor-pointer relative ${
                  darkMode
              ...
              >
                ...
                <input
                  type="text"
                  className={`absolute inset-0 w-full h-full p-1 text-xs outline-none bg-transparent ${
                    darkMode ? "text-gray-100 placeholder-gray-400" : "text-gray-700"
                  }`}
                />
              </div>
            ))}
